...
class WmDetector():
    ...
    def get_pvalues(
        ...
        for ss in scores:
            ...
            pvalues_by_payload = [self.get_pvalue(score, ntoks, eps=eps)[0] for score in scores_by_payload]
            scores_by_payload = [self.get_pvalue(score, ntoks, eps=eps)[1] for score in scores_by_payload]
            pvalues.append(pvalues_by_payload)
            effect_scores.append(scores_by_payload)
        return np.asarray(pvalues), np.asarray(effect_scores) # bsz x payload_max
    ...
    def get_pvalues_by_t(self, scores: List[float]) -> List[float]:
        ...
        for ss in scores:
            cum_score += ss
            cum_toks += 1
            pvalue, score = self.get_pvalue(cum_score, cum_toks)
            pvalues.append(pvalue)
            effect_scores.append(score)
        return pvalues, effect_scores
...
class MinACFDetector(WmDetector):
    ...
    def get_MinACF_pvalues(self, 
        ...
        for ss in scores:
            ...
            # what is ss?
            if ss.shape[-1] == 1:
                flat_score = ss.reshape(-1)

                bar = np.mean(flat_score)

                flat_score_offseted = flat_score - bar

                z_score_compute = np.sum(flat_score_offseted[:-1] * flat_score_offseted[1:]) / (np.sqrt(len(flat_score_offseted) - 1) * self.gamma * (1-self.gamma))

                pvalue = 0.5 * special.erfc(-z_score_compute / np.sqrt(2))
